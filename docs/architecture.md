...
* **`MainActivity`**: Serves as the configuration hub and permission wizard (Overlay, Root). It also hosts the entry point to the `SettingsScreen`.
...
* **`SettingsScreen`**: A full-screen Compose UI for configuring application preferences, managed via `MainActivity`.

### 3. ViewModel Layer
* **`MainViewModel`**: The brain of the operation.
    * **Responsibility**: It bridges the Service and Data layers. It observes `LogcatReader`, pushes data into `StateDelegate`, and applies complex transformations (Regex filtering, User-defined filters, Prohibited tags) before emitting state to the UI. It is a Singleton to allow state sharing between `LogKittyOverlayService` and `MainActivity`/`SettingsScreen`.
...
2.  **Processing**: `MainViewModel` combines the Raw Buffer with `UserPreferences` (Filters, Context Mode, Prohibited Tags).
3.  **Presentation**: `LogBottomSheet` observes the `filteredSystemLog` StateFlow and renders the list using a `LazyColumn`.
